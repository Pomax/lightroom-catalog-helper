...
const routes = {
  ...
  ",index.html": async (req, res) => {
    ...
    res.render("index.html", {
      ...
      missing: req.query.orphans ? await db.getMissing() : undefined,
    });
  },
  ...
  /**
   * Remove all catalog entries for files that no longer exist
   */
  "missing/remove": async (req, res) => {
    let missing = await db.getMissing();
    await db.removeDeletedFiles(missing);
    res.redirect("/?orphans=true");
  }
};
